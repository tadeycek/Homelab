...
function VideoModal({ folder, clip, onClose }: { folder: string; clip: ClipFile; onClose: () => void }) {
  const url = `/api/cf/clips/${encodeURIComponent(folder)}/${encodeURIComponent(clip.name)}`
  useEffect(() => {
...
function SourceModal({ file, onClose }: { file: SourceFile; onClose: () => void }) {
  const url = `/api/cf/source/${encodeURIComponent(file.name)}`
  useEffect(() => {
    const h = (e: KeyboardEvent) => { if (e.key === 'Escape') onClose() }
    window.addEventListener('keydown', h)
    return () => window.removeEventListener('keydown', h)
...
function ClipCard({ folder, clip, onPlay, onDelete, onRegenerate, starred, posted, onToggleStar, onTogglePosted, selectMode, selected, onToggleSelect }: {
  ...
  const url     = `/api/cf/clips/${encodeURIComponent(folder)}/${encodeURIComponent(clip.name)}`
  const thumbUrl= `/api/cf/thumbnails/${encodeURIComponent(folder)}/${encodeURIComponent(clip.name)}`
...
export default function Clips() {
  ...
  const [darkenFactor, setDarkenFactor] = useState(0.6)
  ...
  const refreshSources = useCallback(async () => {
    const data = await fetch('/api/cf/source').then(r => r.json()).catch(() => [])
    setSources(data)
  }, [])

  const refreshDone = useCallback(async () => {
    const data = await fetch('/api/cf/done').then(r => r.json()).catch(() => [])
    setDoneFiles(data)
  }, [])

  const refreshClips = useCallback(async () => {
    const data = await fetch('/api/cf/clips').then(r => r.json()).catch(() => [])
    setFolders(Array.isArray(data) ? data : [])
  }, [])

  const refreshStorage = useCallback(async () => {
    const data = await fetch('/api/cf/storage').then(r => r.json()).catch(() => null)
  ...
  async function cancelJob() {
    ...
    await fetch(`/api/cf/cancel/${activeJobId.current}`, { method: 'POST' })
    appendLog('Cancel requested…', 'info')
  }

  function streamJob(jid: string, onDone: () => void) {
    esRef.current?.close()
    activeJobId.current = jid
    const ev = new EventSource(`/api/cf/stream/${jid}`)
    esRef.current = ev
    ev.onmessage = (e) => {
      ...
      if (d.type === 'log') {
        ...
        ev.close(); esRef.current = null; activeJobId.current = null
        onDone()
      }
    }
    ev.onerror = () => {
      setJobStatus('error'); ev.close(); esRef.current = null; activeJobId.current = null; onDone()
    }
  }
  ...
  async function startDownload() {
    ...
    const { job_id } = await fetch('/api/cf/download', {
  ...
  async function startProcess() {
    ...
    const { job_id } = await fetch('/api/cf/process', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  async function analyzeAudio(file: File) {
    ...
    const fd = new FormData(); fd.append('file', file)
    try {
      const res = await fetch('/api/cf/analyze', { method: 'POST', body: fd }).then(r => r.json())
  ...
  async function analyzeSourceBpm() {
    ...
    try {
      const res = await fetch('/api/cf/analyze-source', { method: 'POST' }).then(r => r.json())
      if (res.bpm) {
  ...
  async function deleteClip(folder: string, name: string) {
    await fetch(`/api/cf/clips/${encodeURIComponent(folder)}/${encodeURIComponent(name)}`, { method: 'DELETE' })
    refreshClips()
  }
  ...
  async function deleteSelectedClips() {
    ...
    await Promise.all(keys.map(key => {
      ...
      return fetch(`/api/cf/clips/${encodeURIComponent(folder)}/${encodeURIComponent(name)}`, { method: 'DELETE' })
    }))
  ...
  async function deleteSource(name: string) {
    await fetch(`/api/cf/source/${encodeURIComponent(name)}`, { method: 'DELETE' })
  ...
  async function deleteDoneFile(name: string) {
    await fetch(`/api/cf/done/${encodeURIComponent(name)}`, { method: 'DELETE' })
    refreshDone()
    refreshStorage()
  }

  async function deleteAllDone() {
    await fetch('/api/cf/done', { method: 'DELETE' })
    refreshDone()
    refreshStorage()
  }
  ...
  async function regenerateClip(folder: string, clipName: string) {
    ...
    if (trimEnd)   body.trim_end   = parseFloat(trimEnd)
    if (beatSync && bpm) { body.bpm = parseFloat(bpm); body.beats_per_cut = parseInt(beatsPerCut) }
    const { job_id } = await fetch('/api/cf/regenerate', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
    }).then(r => r.json())
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div className="card" style={{ padding: '14px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: isMobile ? 12 : 16, flexWrap: 'wrap', marginBottom: 14 }}>
          ...
          <div>
            <Tooltip text="How bright the output clips are. Dark (0.6) = default, Off = no grading applied.">
              <div className="label" style={{ marginBottom: 6, cursor: 'default' }}>Brightness</div>
            </Tooltip>
            <div style={{ display: 'flex', gap: 4 }}>
              {([
                [0.6, 'Dark'],
                ...
                <button key={val} onClick={() => setDarkenFactor(val)} style={{
                  padding: '4px 9px', borderRadius: 5, border: '1px solid',
                  fontSize: 11, fontFamily: 'var(--font-ui)', cursor: 'pointer',
                  borderColor: darkenFactor === val ? 'rgba(129,140,248,0.5)' : 'var(--border)',
                  background: darkenFactor === val ? 'rgba(129,140,248,0.12)' : 'transparent',
                  color: darkenFactor === val ? 'var(--accent)' : 'var(--text-3)', transition: 'all .15s',
                }}>{label}</button>
              ))}
            </div>
          </div>
        ...
        <div className="card" style={{ padding: '14px 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
            ...
            <Tooltip text="Download all clips in this folder as a ZIP file">
              <a
                href={`/api/cf/clips/${encodeURIComponent(latest.folder)}/zip`}
                style={{
                  display: 'inline-flex', alignItems: 'center', gap: 5,
              ...
              return (
                <div key={folder} style={{ borderTop: idx === 0 ? 'none' : '1px solid var(--border)' }}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <a
                      href={`/api/cf/clips/${encodeURIComponent(folder)}/zip`}
